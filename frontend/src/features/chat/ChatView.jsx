...
const EmotionBadge = ({ emotion }) => {
    ...
    const stress = emotion.stress || 0;
    const color = stress > 0.6 ? '#ef4444' : stress > 0.35 ? '#f59e0b' : '#10b981';
    const label = stress > 0.6 ? 'High Stress' : stress > 0.35 ? 'Moderate' : 'Low Stress';
    return (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', background: `${color}15`, color, borderRadius: '20px', padding: '3px 9px', fontSize: '11px', fontWeight: '700', marginTop: '6px' }}>
...
const ChatView = ({ onNavigate }) => {
    ...
    // Load past conversations list — only show those with messages
    useEffect(() => {
    ...
    const startNewSession = async () => {
        ...
        try {
            ...
            // Refresh sidebar list (filter to only conversations with messages)
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            setLastEmotion(res.emotion || null);
            ...
            // Update sidebar count for active conversation
    ...
    return (
        <div style={{ display: 'flex', height: '100%', overflow: 'hidden', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
            {/* Sessions Sidebar */}
            <div style={{ width: '240px', minWidth: '240px', borderRight: '1px solid #f1f5f9', background: 'white', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                ...
                <div style={{ padding: '4px 14px', fontSize: '10px', fontWeight: '700', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Recent Sessions</div>
                {conversations.length === 0 ? (
                    <div style={{ padding: '16px 14px', fontSize: '13px', color: '#94a3b8', textAlign: 'center' }}>No sessions yet</div>
                ) : conversations.map(conv => (
                    <button key={conv.id} onClick={() => setActiveConvId(conv.id)} style={{
                        padding: '10px 14px', border: 'none', cursor: 'pointer', textAlign: 'left', display: 'block', width: '100%',
                ...
                {/* Clinician Connect Card */}
                <div style={{ margin: '12px', marginTop: 'auto', background: '#f0fdf4', borderRadius: '12px', padding: '12px', border: '1px solid #bbf7d0' }}>
                    <div style={{ fontSize: '12px', fontWeight: '700', color: '#059669', marginBottom: '4px' }}>Clinician Mode</div>
                    <div style={{ fontSize: '11px', color: '#374151', marginBottom: '10px', lineHeight: 1.5 }}>Sync your data with your primary therapist for better care.</div>
                    <button style={{ width: '100%', padding: '7px', border: '1.5px solid #6ee7b7', borderRadius: '8px', background: 'white', fontSize: '12px', fontWeight: '600', color: '#059669', cursor: 'pointer' }}>Connect Now</button>
                </div>
            </div>

            {/* Chat Area */}
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#f6f8fa' }}>
                {!activeConvId ? (
                    ...
                    <>
                        {/* Session header */}
                        <div style={{ padding: '10px 20px', background: 'white', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
                        ...
                        {/* Error banner */}
                        {error && (
                            <div style={{ margin: '8px 16px 0', padding: '10px 14px', background: '#fef2f2', borderRadius: '10px', border: '1px solid #fecaca', fontSize: '13px', color: '#dc2626', display: 'flex', justifyContent: 'space-between' }}>
                                {error}
                                ...
                                messages.map((msg) => {
                                    ...
                                    return (
                                        <div key={msg.id} style={{ display: 'flex', justifyContent: isUser ? 'flex-end' : 'flex-start', marginBottom: '12px' }}>
                                            {!isUser && (
                                                <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg, #06b6d4, #3b82f6)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginRight: '8px', flexShrink: 0 }}>
                                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="white"><path d="M12 21.593c-5.63-5.539-11-10.297-11-14.402 0-3.791 3.068-5.191 5.281-5.191 1.312 0 4.151.501 5.719 4.457 1.59-3.968 4.464-4.447 5.726-4.447 2.54 0 5.274 1.621 5.274 5.181 0 4.069-5.136 8.625-11 14.402z" /></svg>
                                                </div>
                                            )}
                                            <div style={{ maxWidth: '68%' }}>
                                                <div style={{
                                                    ...
                                                    padding: '11px 15px', fontSize: '14px', lineHeight: '1.55',
                        ...
                        {/* AI suggestion chips */}
                        {lastSuggestion && (
                            <div style={{ padding: '8px 20px 4px', display: 'flex', gap: '8px', flexWrap: 'wrap', background: '#f6f8fa' }}>
                        ...
                        {/* Input */}
                        <div style={{ padding: '14px 16px', background: 'white', borderTop: '1px solid #f1f5f9', display: 'flex', gap: '10px', alignItems: 'flex-end', flexShrink: 0 }}>
                            ...
                            <button onClick={sendMessage} disabled={!input.trim() || sending || !activeConvId}
                                ...
                                {sending ? <div style={{ width: '14px', height: '14px', border: '2px solid white', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} /> : <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5"><line x1="22" y1="2" x2="11" y2="13" /><polygon points="22 2 15 22 11 13 2 9 22 2" /></svg>}
                            </button>
                        </div>
                    </>
                )}

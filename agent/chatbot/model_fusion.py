...
def analyze_stress_for_tool(text: str) -> str:
    """
    Analyze stress for use in LangChain tools.
    Returns formatted string for agent responses.
    """
    result = calculate_stress_score(text)
...
🎭 **Emotion Detection:**
   ...
   • Top Emotions: {', '.join([f"{e}({s:.0%})" for e, s in result.emotions.top_emotions[:3]])}
...
if __name__ == "__main__":
    print("=" * 60)
    print("MODEL FUSION STRESS CALCULATOR TEST")
    print("=" * 60)
    ...
    test_texts = [
        ...
        "I'm really stressed about my exams, can't sleep at night",
        "Feeling anxious and nervous about the interview tomorrow",
        "I don't know what to do anymore, everything is falling apart",
        "Just had an amazing day with friends, feeling grateful",
        "Why does nobody understand me? I'm so frustrated!",
        "Bahut tension ho raha hai, kya karun samajh nahi aa raha",
        "I feel lonely and sad, missing my family",
    ]
    ...
    for text in test_texts:
        ...
        print(f"💭 Sentiment: {result.sentiment.label} ({result.sentiment.score:.1%})")
        print(f"🎭 Primary Emotion: {result.emotions.primary_emotion} ({result.emotions.primary_score:.1%})")
        print(f"⚠️ Stress Emotions: {result.stress_emotions_detected}")
        print(f"💚 Calming Emotions: {result.calming_emotions_detected}")
        print(f"\n💡 {result.recommendation}")
        print("\n" + "=" * 60 + "\n")
